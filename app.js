...
io.sockets.on('connection', function(socket) {
    ...
    socket.on("imgUpload", function(data) {
        var clients = io.sockets.clients();
        //遍历找到该用户
        clients.forEach(function(client) {
            if (client.name == data.to) {
                console.log(data)
                client.emit('receivedImg', data);
            }
        });
    })
...
server.listen(app.get('port'), function() {
    console.log('Express server listening on port ' + app.get('port') + " and run on " + app.get('env') + " environments");
});